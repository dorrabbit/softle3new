...
let rec eval_exp env = function
  ...
  | LetExp (id, exp1, exp2) ->
       let value = eval_exp env exp1 in (* 現在環境でexp1を評価 *)
       eval_exp (Environment.extend id value env(* envにexp1を追加 *)) exp2
  | FunExp (id, exp) -> ProcV (id, exp, ref env) (* envをクロージャー内に保存 *)
  | AppExp (exp1, exp2) ->
  ...
  | LetRecExp (id, para, exp1, exp2) ->
     ...
     (* 関数閉包を作りidをこの関数閉包に写像するように現在の環境envを拡張 *)
     let newenv = Environment.extend id (ProcV (para, exp1, dummyenv)) env in
     (* ダミー環境への参照に拡張された環境を破壊的代入、バックパッチ *)
     dummyenv := newenv;
